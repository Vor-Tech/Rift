...
router.post("/create", async (req,res) => {
    try {
        ...
        //validate
        if(!owner && !token) return res.status(400).json({msg: "No owner or token provided"});
        ...
        if(!guild_channel && channel_type !== "dedicated" && channel_type !== "forum") return res.status(400).json({msg: "Bad channel type"});
